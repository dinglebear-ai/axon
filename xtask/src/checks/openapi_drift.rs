#[cfg(not(feature = "openapi-drift"))]
pub fn check(_root: &std::path::Path) -> anyhow::Result<()> {
    anyhow::bail!("check-openapi-drift requires building xtask with the `openapi-drift` feature");
}
...
mod enabled {
    ...
    pub fn check(root: &Path) -> Result<()> {
        ...
        if drifted.is_empty() {
            ...
            super::super::android_api_contract::check_against_openapi(root)?;
            return Ok(());
        }
    ...
    fn command_exists(root: &Path, program: &str) -> Result<bool> {
        ...
        match status {
            ...
            Err(error) => {
                Err(error).with_context(|| format!("failed to invoke `{program} --version`"))
            }
        }
    }
