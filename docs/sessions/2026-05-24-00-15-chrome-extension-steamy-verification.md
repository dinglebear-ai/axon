...
# Chrome Extension Winhost Verification

## User Request

Diagnose and resolve the Axon Chrome extension failure shown on Winhost, then verify that it actually works rather than relying on inferred backend checks.
...
- Updated the Winhost Chrome extension storage to use the current Axon token and verified the exact installed extension origin reaches Axon auth.
...
- Could not physically drive the already-open Winhost Chrome side panel because CDP was not listening on Winhost, WSL interop was disabled, and Windows-MCP tools were not exposed in this Codex session.
...
2. Read the Winhost Chrome extension storage LevelDB for extension ID `kllododgecpcdimlgiliginoicbgmeif`, then updated `axonUrl`, `axonToken`, and `autoScrapeEnabled`.
...
4. Patched SWAG nginx on `edgehost` to allow `chrome-extension://[a-p]{32}` origins, validated with `nginx -t`, and reloaded SWAG.
...
7. Checked Winhost and agent-os control surfaces after the user challenged the verification claim.
...
- SWAG/nginx, not Axon itself, caused the extension-origin `403`; allowing `chrome-extension://...` origins changed the exact Winhost extension origin from forbidden to an auth-passed `400` on the intentional empty scrape.
- Winhost automation remained blocked: `curl http://127.0.0.1:9222/json/version` and `:9223` failed, `/mnt/c/Windows/System32/cmd.exe /c ver` returned `exec format error`, and no `mcp__windows-mcp__*` tools were available.
...
- Kept the final claim scoped: backend/proxy/extension UI flow verified, but the already-open Winhost Chrome window not physically driven.
...
| created | `docs/sessions/2026-05-24-chrome-extension-winhost-verification.md` | | Capture this verification and follow-up session | this file |
...
| modified | `C:\Users\jmaga\axon-chrome-extension\*` | | Deployed unpacked extension files to Winhost | `rsync -a --delete -e ssh apps/chrome-extension/ winhost-wsl:/mnt/c/Users/jmaga/axon-chrome-extension/` |
...
- Shell commands: SSH to `winhost-wsl`, SSH to `devhost`, curl probes, git state inspection, `bd list`, Node smoke scripts, and LevelDB reads/writes via `classic-level`.
- External CLIs/services: SWAG nginx inside Docker on `edgehost`, Chromium with extension loading, Xvfb, Axon HTTP API at `https://axon.example.internal`.
- Browser tools: Chrome DevTools Protocol was available for the automated Chromium smoke test, but not for the already-open Winhost Chrome session.
...
| command | result |
| --- | --- |
| `ssh winhost-wsl 'curl -fsS --max-time 2 http://127.0.0.1:9222/json/version \|\| true; curl ...:9223...'` | Both Winhost CDP checks failed to connect. |
| `ssh winhost-wsl '/mnt/c/Windows/System32/cmd.exe /c ver'` | Failed with `exec format error`; WSL interop was disabled. |
| `curl -X POST https://axon.example.internal/v1/scrape -H 'Origin: chrome-extension://kllododgecpcdimlgiliginoicbgmeif' ... --data '{}'` | Returned `400 {"kind":"bad_request","message":"url or urls is required"}`, proving auth reached Axon. |
...
| `rsync -a --delete -e ssh apps/chrome-extension/ winhost-wsl:/mnt/c/Users/jmaga/axon-chrome-extension/` | Deployed current extension files to Winhost unpacked-extension directory. |
...
- Live Winhost UI automation was blocked by unavailable CDP, disabled WSL interop, and missing Windows-MCP tools.
...
| Side panel showed repeated forbidden/auth failures against `https://axon.example.internal`. | Extension UI smoke test successfully asked Axon and received a sourced answer. |
...
| Exact Winhost origin curl to `/v1/scrape` with empty body | `400` bad request after auth, not `401` or `403` | `400 {"kind":"bad_request","message":"url or urls is required"}` | pass |
...
| Winhost CDP check on `9222`/`9223` | CDP endpoint available for live UI automation | Connection refused | blocked |
| Winhost Windows interop check via `cmd.exe` | Windows command runner usable from WSL | `exec format error` | blocked |
...
- The already-open Winhost Chrome profile may need an extension reload to pick up deployed unpacked-extension changes if Chrome has not reloaded the unpacked extension.

## Decisions Not Taken

- Did not claim the already-open Winhost side panel was physically verified, because no working control channel existed.
...
- Whether the already-open Winhost Chrome extension instance has reloaded the deployed unpacked-extension files.
- Whether the next Codex/Claude session can expose `mcp__windows-mcp__*` tools or Winhost Chrome CDP so the live personal browser can be driven directly.

## Next Steps

- For final live Winhost proof, launch Chrome with remote debugging on Winhost or expose Windows-MCP tools, then drive the installed side panel and send an `ask` message in that exact browser session.
- If the current visible Winhost Chrome still shows old behavior, reload the unpacked extension from `chrome://extensions` or restart the debug Chrome session and re-run the side-panel check.
