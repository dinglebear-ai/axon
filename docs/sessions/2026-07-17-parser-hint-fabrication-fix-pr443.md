...
- Local axon test runs on devhost are poisoned by the live `~/.axon` (jobs.db schema epoch + config.toml values leak into `ServiceContext`); isolated `AXON_DATA_DIR`/`AXON_CONFIG_PATH` is mandatory, and even then one shared tmpdir across a parallel `--workspace` run produces order-dependent flakes (`axon_rust-29tql`).
