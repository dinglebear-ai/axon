...
| `--max-pages <n>` | `1` when omitted, `0` when explicitly uncapped | Passed to extract web runner as the page traversal limit. |
...
LLM fallback extraction runs through the configured LLM backend. Gemini headless
is the default; configure its model with `AXON_SYNTHESIS_HEADLESS_GEMINI_MODEL`
(legacy alias: `AXON_HEADLESS_GEMINI_MODEL`) and the binary with
...
axon extract https://example.com --query "extract title" --json
```
...
- Generic CLI client-to-server forwarding was removed in 5.0.0. `AXON_SERVER_URL` does not route `axon extract` through HTTP; call the `/v1/extract` REST route or MCP HTTP endpoint directly when using `axon serve` as a remote service.
