# Chrome Extension Winhost Verification Artifact

Generated: 2026-05-24

Source session: `docs/sessions/2026-05-24-chrome-extension-winhost-verification.md`
...
The already-open Winhost Chrome side panel was not physically driven. That final live-browser proof remains blocked until Winhost Chrome exposes CDP or Windows-MCP tools are available.
...
- The exact Winhost extension origin `chrome-extension://kllododgecpcdimlgiliginoicbgmeif` reached Axon auth after the SWAG change.
...
- The already-open personal Winhost Chrome side panel was not directly exercised.
- It is not proven that the currently visible Winhost Chrome instance has reloaded the deployed unpacked-extension files.
...
| Winhost CDP probes on ports `9222` and `9223` | Connection refused. |
| Winhost WSL Windows interop probe via `cmd.exe` | Failed with `exec format error`. |
...
Launch or expose a controllable Winhost Chrome session, then drive the installed side panel in that exact browser profile and submit an Axon question.

Acceptable control paths:

- Start Winhost Chrome with remote debugging enabled and connect to CDP.
...
Pass condition: the installed Winhost side panel sends an ask request and displays a sourced Axon answer without `401` or `403` errors.
