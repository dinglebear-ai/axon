...
| Product version | `7.2.7` in `[workspace.package]`, inherited by every crate via `version.workspace = true` |
| MCP runtime | `rmcp = "=3.1.0"` — exact pin, agrees with `Cargo.lock` |
...
`docs/reference/cli/commands.md` (rendered). It currently holds **109 commands
across 49 groups**, regenerated by `cargo xtask schemas cli`. Do not
...
| Retrieval and analysis | `query`, `retrieve`, `ask`, `chat`, `summarize`, `evaluate`, `train`, `suggest`, `search`, `research`, `extract` plus its generated lifecycle conveniences, `brand`, `diff`, `endpoints`, `screenshot` |
...
| `axon-chrome` | built from config/chrome/Dockerfile | `6000` (management), `9222` (CDP proxy), `9223` (raw DevTools) | headless_browser + chrome-headless-shell |
...
```bash
just verify      # full pre-PR gate, in order: legacy-runtime-check,
                 # blocking-async-check, primitive-inventory-check,
                 # validate-plugin, web-check, fmt-check, clippy, machete,
                 # check, test
just fix         # cargo fmt --all + clippy --fix --all-targets (auto-repair)
just precommit   # staged-file gate: compose port bindings, no-legacy-symbols,
                 # legacy-runtime-check, primitive-inventory-check, monolith
...
schema. Its current clean-break epoch is 1 and it contains 31 domain tables
parsed from 10 canonical migration files. Keep this summary grouped by ownership; use the JSON for
...
| Source ledger | `sources`, `source_generations`, `source_manifests`, `source_items`, `document_status`, `source_publication_state`, `graph_publication_state`, `cleanup_debt`, `leases` |
...
| Memory | `memory_records`, `memory_links`, `memory_reinforcement`, `memory_reviews` |
