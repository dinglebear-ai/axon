...
6. Use `axon <url> --scope site --render-mode chrome --automation-script ... --wait true` for repeatable rendered capture before escalating to browser/link probing for broken links, console/network failures, or rendered-only SEO evidence.
7. Axon has **no robots.txt enforcement flag** — bound public-site capture with `--max-pages`, `--max-depth`, and `--budget PATH=N`, and honor the site's terms.
