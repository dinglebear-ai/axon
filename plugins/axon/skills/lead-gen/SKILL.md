...
For finished lead-list deliverables, run Axon commands in blocking mode: `axon extract ... --wait true --json` and `axon <source> --wait true` (add `--scope site` for a directory section). All indexing goes through that one source command — there is no `axon crawl` or `axon ingest`. Axon has **no robots.txt enforcement flag**; keep public-source capture bounded with `--max-pages`, `--max-depth`, and `--budget PATH=N`, and honor the source's terms.
