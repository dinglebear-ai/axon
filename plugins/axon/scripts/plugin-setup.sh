...
main() {
  ...
  export_if_set AXON_HTTP_TOKEN CLAUDE_PLUGIN_OPTION_API_TOKEN
  ...
  export_if_set AXON_AUTH_MODE CLAUDE_PLUGIN_OPTION_AUTH_MODE
  export_if_set AXON_PUBLIC_URL CLAUDE_PLUGIN_OPTION_PUBLIC_URL
  export_if_set AXON_GOOGLE_CLIENT_ID CLAUDE_PLUGIN_OPTION_GOOGLE_CLIENT_ID
  export_if_set AXON_GOOGLE_CLIENT_SECRET CLAUDE_PLUGIN_OPTION_GOOGLE_CLIENT_SECRET
  export_if_set AXON_AUTH_ADMIN_EMAIL CLAUDE_PLUGIN_OPTION_AUTH_ADMIN_EMAIL

  mkdir -p "${AXON_HOME}"
  chmod 700 "${AXON_HOME}" 2>/dev/null || true
  warn_stale_systemd_unit
  ensure_axon_binary || exit 0
