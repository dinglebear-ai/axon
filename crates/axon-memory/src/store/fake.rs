//! In-memory `MemoryStore` fake for unit tests that don't need SQLite.
...
use super::{MemoryStore, Result, unsupported_option};
...
/// Recall visibility for the fake store: forgotten never returns; superseded
/// only when explicitly requested via `include_statuses`; archived only when
/// `include_archived` or explicitly requested (contract "Scoring and Recall").
fn recall_visible(
    record: &MemoryRecord,
