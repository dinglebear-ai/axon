-- Widen the durable provider scheduler's provider_kind CHECK to the full
-- current ProviderKind registry (axon-api source/enums/runtime.rs). The 0004
-- CHECK predates the ledger/graph/memory/job/watch/config/credential/security/
-- rate_limiter/health_probe capacity domains, so reservations for those kinds
-- (e.g. the sqlite-graph scheduler behind baseline graph upserts) failed with
-- "CHECK constraint failed: provider_kind IN (...)" and degraded their
-- summaries. 'storage' stays allowed for rows written before the enum dropped
-- it; the INSERT..SELECT below must re-admit every historical row.
-- SQLite cannot alter CHECK constraints in place, so rebuild the table
-- append-only while preserving every scheduler column, foreign key, row, and
-- index from migrations 0001-0004.
CREATE TABLE provider_reservations_v9 (
    reservation_id TEXT PRIMARY KEY NOT NULL,
    job_id TEXT NOT NULL REFERENCES jobs(job_id) ON DELETE CASCADE,
    stage_id TEXT REFERENCES job_stages(stage_id) ON DELETE SET NULL,
    provider_kind TEXT NOT NULL CHECK (provider_kind IN ('embedding', 'vector', 'llm', 'fetch', 'render', 'search', 'storage', 'cache', 'network_capture', 'artifact', 'parser', 'ledger', 'graph', 'memory', 'job', 'watch', 'config', 'credential', 'security', 'rate_limiter', 'health_probe')),
    provider_id TEXT,
...
INSERT INTO provider_reservations_v9 (
...
ALTER TABLE provider_reservations_v9 RENAME TO provider_reservations;
