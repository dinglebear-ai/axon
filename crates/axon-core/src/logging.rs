...
// file-log events through the same `redact_event_fields` scrub as the console
// layer — the built-in JSON formatter has no redaction hook.
...
fn should_use_ansi_for_writer(writer_supports_ansi: bool) -> bool {
    ...
    if crate::ui::color_forced_never() {
        return false;
    }
    ...
    if crate::ui::color_forced_always() {
        return true;
    }
    if crate::ui::color_env_disabled() {
        return false;
    }
    // --color=auto: FORCE_COLOR/CLICOLOR_FORCE still wins over TTY detection.
    if crate::ui::color_env_forced() {
        return true;
    }
