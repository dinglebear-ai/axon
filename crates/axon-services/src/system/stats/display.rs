//! Human-readable `axon stats` rendering.
//!
//! Ports legacy `axon-vector`'s `ops::stats::display` verbatim — pure
//! presentation over an already-gathered `serde_json::Value`, with zero
//! Qdrant/SQLite dependency, so the migration off `axon_vector::` is a
//! straight relocation, not a rewrite. Called by [`super::print_stats_human`].
