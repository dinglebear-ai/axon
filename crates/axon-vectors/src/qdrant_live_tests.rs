...
async fn cleanup_at(url: &str, collection: &str) -> Result<(), reqwest::Error> {
...
async fn finish_fixture<F: Future<Output = ()>>(url: &str, collection: &str, body: F) {
    let outcome = AssertUnwindSafe(body).catch_unwind().await;
    let cleanup = cleanup_at(url, collection).await;
    ...
    cleanup.expect("delete fixture collection");
}
...
async fn qdrant_live_ensure_upsert_search_round_trip() {
    ...
    finish_fixture(&live_url(), &collection, async {
...
#[tokio::test]
async fn fixture_deletes_collection_even_after_panic() {
    let server = httpmock::MockServer::start();
    let deleted = server.mock(|when, then| {
        when.method(httpmock::Method::DELETE)
            .path("/collections/fixture");
        then.status(200)
            .json_body(serde_json::json!({"result":true}));
    });
    let outcome = AssertUnwindSafe(finish_fixture(&server.base_url(), "fixture", async {
