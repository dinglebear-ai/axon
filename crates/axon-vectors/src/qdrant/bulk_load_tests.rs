...
async fn credential_and_query_aliases_share_bulk_load_ownership() {
    ...
    let first_url = format!("http://first:secret@{host}:{port}/?api_key=ignored"); // gitleaks:allow — synthetic credential fixture
    let second_url = format!("http://second:secret@{host}:{port}?api_key=other#fragment"); // gitleaks:allow — synthetic credential fixture
    let lower = server
        .mock_async(|when, then| {
