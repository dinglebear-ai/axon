...
Android Back is routed through Tauri's native back-button listener into the same Palette unwind order used by desktop Escape: close Settings, unwind History/Browse, clear action/query state, then exit the app only at the true root. Root exit goes through the Tauri app lifecycle so `RunEvent::Exit` cleanup still executes.
