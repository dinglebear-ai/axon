...
(function () {
  ...
  function Card(children, style) {
    return el("div", { style: { borderRadius: 8, background: "var(--axon-control)", border: "1px solid var(--aurora-border-default)", padding: "12px 13px", boxShadow: "var(--aurora-highlight-medium)", ...(style || {}) } }, children);
  }
  ...
  function MdBlocks(blocks, tones) {
    return el("div", { style: { display: "flex", flexDirection: "column", gap: 11 } }, (blocks || []).map((b) => {
      if (b.t === "h1") return el("h1", { style: { margin: 0, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 22, letterSpacing: 0, color: "var(--aurora-text-primary)", lineHeight: 1.12 } }, b.x);
      ...
      if (b.t === "code") return el("div", { style: { borderRadius: 8, overflow: "hidden", border: "1px solid var(--aurora-border-default)", background: "var(--axon-page)" } }, [
  ...
  function StatsGrid(data, tones) {
    ...
    return col([
      el("div", { style: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 9 } }, cells.map((c) => Card([
        ...
        el("div", { style: { marginTop: 5, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 20, letterSpacing: 0, fontVariantNumeric: "tabular-nums", color: c.color || "var(--aurora-text-primary)" } }, c.value),
      ], { padding: "11px 12px" }))),
  ...
  function JobAccepted(data, op, tones) {
    ...
    return Card([
      el("div", { style: { display: "flex", alignItems: "center", gap: 10 } }, [
        el("span", { style: { width: 36, height: 36, flex: "0 0 36px", borderRadius: 8, display: "grid", placeItems: "center", color: "var(--axon-orange-strong)", background: tint("var(--axon-orange)", 14, "var(--axon-page)"), border: `1px solid ${tint("var(--axon-orange)", 30)}` } }, Icon(op.icon, 18)),
        el("div", null, [
  ...
  function BrandPalette(data, tones) {
    return col([
      ...
      el("div", { style: { display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 } }, data.colors.map((c) => el("div", { style: { display: "flex", flexDirection: "column", gap: 5 } }, [
        el("div", { style: { height: 46, borderRadius: 8, background: c.hex, border: "1px solid var(--aurora-border-default)" } }),
  ...
  function ScreenshotView(data, tones) {
    return col([
      ...
      el("div", { style: { borderRadius: 8, overflow: "hidden", border: "1px solid var(--aurora-border-default)", background: "var(--axon-page)", aspectRatio: "3 / 4", maxHeight: 340, display: "grid", placeItems: "center" } }, [
  ...
  function GenericResult(op, raw) {
    ...
    return col([
      ...
      el("div", { style: { borderRadius: 8, overflow: "hidden", border: "1px solid var(--aurora-border-default)", background: "var(--axon-page)" } }, [
